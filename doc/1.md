...
2. 原始客户端，包含 Android 和 iOS 两个平台，根据自己需求决定。
3. 原始全量数据包，同上。
4. Git，用于同步程序更新。
...
Linux 系统直接用包管理工具安装即可，以下为 Windows 用户需要的下载地址。
 ...
 - SIF 客户端: 链接: https://pan.baidu.com/s/1CRxw1JtEDRRzIxknXYMIaA?pwd=sif1 提取码: sif1 或 https://mercari-my.sharepoint.com/:f:/g/personal/sakana_mercari_onmicrosoft_com/IgBUbQY4zT3oTZHSkULr3z6pAcfgK-k9OdvXTa8ZkqpoObY 两个链接内容相同，任选其一下载即可。
 - SIF 全量数据: 同 SIF 客户端下载地址。数据包目录中的 list_CN_Android 和 list_CN_iOS。
